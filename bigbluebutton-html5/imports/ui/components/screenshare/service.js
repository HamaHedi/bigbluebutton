...
export const DEFAULT_SCREENSHARE_STATS_TYPES = ["outbound-rtp", "inbound-rtp"];
...
export const useSharingContentType = () =>
  useReactiveVar(sharingContentTypeVar);
export const useCameraAsContentDeviceIdType = () =>
  useReactiveVar(cameraAsContentDeviceIdTypeVar);
...
export const getCameraAsContentDeviceId = () =>
  cameraAsContentDeviceIdTypeVar();
...
export const useIsCameraAsContentGloballyBroadcasting = () => {
  ...
  return Boolean(
    data &&
    data[0] &&
    data[0].contentType === CONTENT_TYPE_CAMERA &&
    data[0].stream,
  );
};
...
export const getMediaElement = () =>
  document.getElementById(SCREENSHARE_MEDIA_ELEMENT_NAME);
...
export const setOutputDeviceId = (outputDeviceId) => {
  ...
  const sinkIdSupported =
    screenShareElement && typeof screenShareElement.setSinkId === "function";
...
export const shareScreen = async (
  ...
  try {
    ...
    if (options.stream == null) {
      ...
      // Allow explicit contentType in options, default to CAMERA for backward compatibility
      contentType = options.contentType || CONTENT_TYPE_CAMERA;
...
export const viewScreenshare = (streamId, hasAudio, options = {}) => {
  screenShareBridge
    .view(streamId, { hasAudio, outputDeviceId: options.outputDeviceId })
    .catch((error) => {
      logger.error(
        {
...
export const screenshareHasStarted = (
  streamId,
  hasAudio,
  isPresenter,
  options = {},
...
export const screenShareEndAlert = () =>
